...
fitLineColor = ROOT.kRed
fitLineWidth = 3

def FitAndDraw(histo, littleLabel = "", doFit = 1):
  print "doFit:", doFit
...
def Fit1DHLine(histo):
  ...
  fit.SetLineColor(fitLineColor)
  fit.SetLineWidth(fitLineWidth)
...
def Fit1DGauss(histo):
  ...
  fit1.SetLineColor(fitLineColor)
  fit1.SetLineWidth(fitLineWidth)
  ...
  fit2.SetLineColor(fitLineColor)
  fit2.SetLineWidth(fitLineWidth)
...
#  sNorm = "Mean = " + str(mean2) + "#pm" + str(mean2_e) % mean2, mean2_e
  ...
  fitLegend.SetTextColor(fitLineColor)
